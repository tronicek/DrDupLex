...
/**
 * The linearizations in the compressed persistent TRIE.
 * 
...
public class CPLinearizations {
    ...
    public static void initialize(Storage storage) throws IOException {
        ...
        try (FileChannel ch = FileChannel.open(labelFile.toPath(), StandardOpenOption.CREATE, StandardOpenOption.WRITE)) {
            ch.truncate(0);
        }
        File linFile = storage.getLinearizationFile();
        try (FileChannel ch = FileChannel.open(linFile.toPath(), StandardOpenOption.CREATE, StandardOpenOption.WRITE)) {
            ch.truncate(0);
        }
    }
    ...
    public void extendBuffer(List<String> labs) {
        for (int i = buffer.size(); i < labs.size(); i++) {
    ...
    public void store(Storage storage) throws IOException {
        ...
        try (DataOutputStream out = new DataOutputStream(new FileOutputStream(storage.getLinearizationFile()))) {
