...
/**
 * The labels in the compressed persistent TRIE.
 * 
 * @author Zdenek Tronicek
 */
public class CPLabels {

    private final List<String> labels;

    private CPLabels(List<String> labels) {
        this.labels = labels;
    }
    ...
    public static CPLabels load(Storage storage) throws IOException {
        List<String> labels = new ArrayList<>();
        File labelFile = storage.getLabelFile();
        if (!labelFile.exists()) {
            return new CPLabels(labels);
        }
    ...
    public void print(Storage storage) throws IOException {
        int index = 0;
        try (DataInputStream in = new DataInputStream(new FileInputStream(storage.getLabelFile()))) {
            try {
                while (true) {
                    String label = in.readUTF();
