...
public class CPNextStmtMap {
    ...
    private void append(Storage storage, Map<Pos, Pos> map) throws IOException {
        try (DataOutputStream out = new DataOutputStream(new FileOutputStream(storage.getNextStmtMapFile(), true))) {
            for (Pos stmt : map.keySet()) {
                Pos next = map.get(stmt);
    ...
    public static void initialize(Storage storage) throws IOException {
        ...
        if (mapFile != null) {
            try (FileChannel ch = FileChannel.open(mapFile.toPath(), StandardOpenOption.CREATE, StandardOpenOption.WRITE)) {
                ch.truncate(0);
            }
        }
    }
